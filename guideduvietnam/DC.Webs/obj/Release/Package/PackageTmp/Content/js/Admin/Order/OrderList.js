var viewModel = new function () {
    ...
    self.StatusConfirmOptions = ko.observableArray(data.StatusConfirmOptions != null ? data.StatusConfirmOptions : []);
    self.OrderItems = ko.observableArray([]);
    self.OrderDetailItems = ko.observableArray([]);
    self.StatusConfirm = ko.observable('COMPLETE');
    // Paginate
    ...
    function FilterData() {
        ...
        $.ajax({
            ...
            success: function (data, textStatus, jqXHR) {
                ...
                viewModel.OrderItems(data.OrderItems == null ? [] : data.OrderItems);
            },
    ...
    self.ViewOrders = function (item) {
        ...
        $.ajax({
            ...
            success: function (data, textStatus, jqXHR) {
                hideProcess();
                // Set values               
                viewModel.OrderDetailItems(data.OrderDetailItems == null ? [] : data.OrderDetailItems);
            },
            error: function (xhr, status, error) {
                // Stop progress bar
                hideProcess();
        ...
        $('#popupOrderDetail').modal('show');

    };

    self.UpdateStatus= function(item) {
        viewModel.ItemId(item.Id);
        $('#popupUpdateStatus').modal('show');
    }

    self.UpdateStatusConfirm= function() {
        var dataPost = { "orderId": viewModel.ItemId(), "status":viewModel.StatusConfirm() };
        $.ajax({
            type: "POST",
...
ko.applyBindings(viewModel, document.getElementById("orderList"));
//// Onload
// jQuery
